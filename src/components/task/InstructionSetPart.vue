...
<div class="instruction-set-part" @keydown.ctrl="keyboardSave($event)">
  ...
  <div class="editor-area" v-loading="script === null" :element-loading-text="$t(lang + 'loading_script')">
    <codemirror v-model="script" :options="options" v-if="script !== null" class="instruction-set-editor"></codemirror>
  </div>
  <el-button @click="saveScript(not_save_instruction_set_key)" type="success" class="editor-save" icon="el-icon-check" circle></el-button>
</div>
</template>
